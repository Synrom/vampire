...
bool CodeTree::Matcher<removing, checkRange>::prepareLiteral()
{
  ...
  if (const Checkpoint* cp = cpCursor.next(linfoCnt)) {
    ASS(bindings.size() >= cp->bindings.size());
    for (unsigned i=0; i < cp->bindings.size(); i++) {
      bindings[i] = cp->bindings[i];
    }
    boundedSize = (unsigned)cp->bindings.size();
    curLInfo = cp->liIndex;
    ft = linfos[curLInfo].ft;
    tp = cp->tp;
    op = cpCursor.entry;
...
inline void CodeTree::Matcher<removing, checkRange>::doNextOp()
{
  ...
  // save this checkpoint's own copy of the bindings
  BindingArray saved;
  saved.ensure(boundedSize);
