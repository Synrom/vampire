...
#ifndef __CodeTree_Ablation_SaveBindingsDirectly__
#define __CodeTree_Ablation_SaveBindingsDirectly__
...
public:
    ...
    : public std::conditional<removing, RemovingBase, NonRemovingBase>::type
  {
     ...
     * The bindings are stored directly on the checkpoint itself.
     */
    struct Checkpoint
    {
      unsigned liIndex;
      ...
      Checkpoint(unsigned liIndex_, unsigned tp_, BindingArray bindings_)
        : liIndex(liIndex_), tp(tp_), bindings(std::move(bindings_)) {}
    };
...
#endif // __CodeTree_Ablation_SaveBindingsDirectly__
