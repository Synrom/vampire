...
Clause* ClauseCodeTree::ClauseMatcher::next(int& resolvedQueryLit)
{
  ...
  struct RecordOpCounts {
    ~RecordOpCounts() {
      RSTAT_CTR_INC_MANY("executed code ops saveBindingsDirectly", CodeTree::executedOpsCount);
      RSTAT_CTR_INC_MANY("executed next ops saveBindingsDirectly", CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC("clause matcher calls saveBindingsDirectly");
    }
  } recordOpCounts;
  if(lms.isEmpty()) {
    return 0;
