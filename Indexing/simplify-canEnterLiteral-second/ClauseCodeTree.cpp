...
Clause* ClauseCodeTree::ClauseMatcher::next(int& resolvedQueryLit)
{
  ...
  struct RecordOpCounts {
    ~RecordOpCounts() {
      RSTAT_CTR_INC_MANY("executed code ops simplifyCanEnterSecond", CodeTree::executedOpsCount);
      RSTAT_CTR_INC_MANY("executed next ops simplifyCanEnterSecond", CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC("clause matcher calls simplifyCanEnterSecond");
    }
  } recordOpCounts;
  if(lms.isEmpty()) {
    return 0;
