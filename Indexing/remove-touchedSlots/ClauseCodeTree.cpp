...
{
  TIME_TRACE("Clause Matcher next removeTouchedSlots")
  CodeTree::executedOpsCount = 0;
  CodeTree::executedNextOpsCount = 0;
  struct RecordOpCounts {
    ~RecordOpCounts() {
      RSTAT_CTR_INC_MANY("executed code ops removeTouchedSlots", CodeTree::executedOpsCount);
      RSTAT_CTR_INC_MANY("executed next ops removeTouchedSlots", CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC("clause matcher calls removeTouchedSlots");
    }
  } recordOpCounts;
  if(lms.isEmpty()) {
    return 0;
