...
{
  TIME_TRACE("Clause Matcher next ilstructSimplification")
  CodeTree::executedOpsCount = 0;
  CodeTree::executedNextOpsCount = 0;
  struct RecordOpCounts {
    ~RecordOpCounts() {
      RSTAT_CTR_INC_MANY("executed code ops ilstructSimplification", CodeTree::executedOpsCount);
      RSTAT_CTR_INC_MANY("executed next ops ilstructSimplification", CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC("clause matcher calls ilstructSimplification");
    }
  } recordOpCounts;
  if(lms.isEmpty()) {
    return 0;
