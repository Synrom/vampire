...
{
  TIME_TRACE("Clause Matcher next ordered")
  ...
  struct RecordOpCounts {
    ~RecordOpCounts() {
      RSTAT_CTR_INC_MANY("executed code ops ordered", CodeTree::executedOpsCount);
      RSTAT_CTR_INC_MANY("executed next ops ordered", CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC_MANY("executed code ops incl next ordered",
          CodeTree::executedOpsCount + CodeTree::executedNextOpsCount);
      RSTAT_CTR_INC("clause matcher calls ordered");
      RSTAT_CTR_INC_MANY("NEXT checkpoints recorded ordered", CodeTree::recordedCheckpointsCount);
      RSTAT_CTR_INC_MANY("NEXT checkpoints used ordered", CodeTree::usedCheckpointsCount);
      for (unsigned len : CodeTree::executedNextOverlapLens) {
        RSTAT_MCTR_INC("NEXT overlap length ordered", len);
      }
    }
  } recordOpCounts;
  if(lms.isEmpty()) {
